...
type ModalProps = {
  ...
  item: HistoryTransfers;
};

const StockTransferViewModal = ({ item, setModal }: ModalProps) => {
  useEffect(() => {
        ...
        <div className="mt-8 overflow-y-auto h-[200px] p-3">
          ...
          {item.items.map((transfer, index) => (
            <>
              ...
              {transfer.reason && (
                <div className="mt-4">
                  <h2 className="text-[18px]">REASON</h2>
                  <p className="text-[16px] text-[#494A4B]">
                    {transfer.reason}
                  </p>
                </div>
              )}
...
export default StockTransferViewModal;
